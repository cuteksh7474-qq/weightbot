...
st.caption("치수 자동(OCR) 실패 시 **박스 치수 수동 입력** 또는 **강제 적용**을 사용하세요.")
...
with colx:
    if st.button("🧠 OCR로 채우기(강화)"):
        ...
        if d:
            ...
            st.warning("OCR에서 치수 미검출. 아래 보조 입력을 사용하세요.")
with coly:
    force_txt = st.text_input("보조 입력(예: 750mm, 640mm, 550mm / 75cm,55cm,64cm)")
    if st.button("➡ 강제 적용"):
...
with st.expander("🔎 OCR 원문/진단 보기"):
    st.write(f"**OCR 사용 가능:** {HAS_OCR}")
    st.text(st.session_state.get("__ocr_text__", "아직 실행되지 않았습니다."))

# ---- Options ----
st.subheader("옵션명(선택, 한 줄에 하나) + 출력용량 선택")
...
def estimate_wrap(name, power_choice):
    ...
    return estimate(product_name,cap,auto_cat,dims_used,power_kw,db=db)
