...
# ---------- Rules ----------
CATEGORY_KEYWORDS = {
    "small_elec": ["드라이기", "청소기", "电子", "small", "vacuum", "吸尘器"],
    "rice_cooker": ["밥솥", "电饭煲"],
    "kettle": ["주전자", "电热水壶"],
    "thermos": ["보온병", "保温壶"],
    "air_fryer": ["에어프라이어", "空气炸锅"],
    "blender": ["믹서기", "破壁机"],
    "shoes": ["신발", "鞋"],
    "clothing": ["의류", "衣服"],
    "container": ["용기", "塑料", "收纳"],
...
def dims_pattern(txt):
    ...
    m = re.search(
        r"(\d+(?:\.\d+)?)\s*[x\*]\s*(\d+(?:\.\d+)?)\s*[x\*]\s*(\d+(?:\.\d+)?)(\s*(mm|cm|m|毫米|厘米|米))?",
        s,
        re.I,
    )
    if m:
        a, b, c = float(m.group(1)), float(m.group(2)), float(m.group(3))
        unit = (m.group(5) or "cm")
        L, W, H = sorted([u2cm(a, unit), u2cm(b, unit), u2cm(c, unit)], reverse=True)
        return (L, W, H)
    m2 = re.search(
        r"长\s*(\d+(?:\.\d+)?)\s*(mm|cm|m|毫米|厘米|米)?\s*宽\s*(\d+(?:\.\d+)?)\s*(mm|cm|m|毫米|厘米|米)?\s*高\s*(\d+(?:\.\d+)?)\s*(mm|cm|m|毫米|厘米|米)?",
        s,
        re.I,
    )
    if m2:
        L = u2cm(float(m2.group(1)), (m2.group(2) or "cm"))
        W = u2cm(float(m2.group(3)), (m2.group(4) or "cm"))
        H = u2cm(float(m2.group(5)), (m2.group(6) or "cm"))
        return (L, W, H)
    return None
...
def dims_anywhere(txt):
    ...
    dedup = []
    for v in vals:
        if not any(abs(v - d) < 1.0 for d in dedup):
            dedup.append(v)
...
def ocr_text_from_images(files):
    if not (HAS_OCR and files):
        return "", "OCR 비활성 또는 이미지 없음"
    reader = build_reader()
    chunks = []
    for f in files:
        try:
...
def estimate(product_name, cap_L, cat, dims, power_kw=0.0, extra=0.1, net_override=None, db=None):
    ...
    if net_override is not None:
        ...
        cap = max(0.0, cap_L or 0.0)
        pr = PRI[cat]
        net = pr["shell_per_L"] * cap + pr["inner_per_L"] * cap + pr["base"] + pr["acc"] + extra
    elif cat == "container":
        cap = cap_L or 0.0
        if cap <= 0 and all(dims):
            cap = (L * W * H * 0.6) / 1000.0
        ...
        net = PRI["clothing"]["piece"]
    elif cat == "pot_pan":
        net = PRI["pot_pan"]["base"] + extra
    elif cat == "beauty":
        net = PRI["beauty"]["base"] + extra
    else:
        net = 0.6 + extra
    net += max(0.0, power_kw) * POWER_FACTORS.get(cat, 0.3)
    delta = avg_delta(db or {}, cat)
    net = max(0.05, net + delta)
    return {
        "net_kg": round(net, 2),
        "gross_kg": round(net + pkg_w(L, W, H), 2),
        "vol_5000": round(vol(L, W, H, 5000), 2),
        "vol_6000": round(vol(L, W, H, 6000), 2),
        "confidence": min(95, 70 + (10 if cap_L > 0 else 0) + (10 if all(dims) else 0) + (5 if power_kw > 0 else 0)),
        "category": cat,
        "delta_applied": round(delta, 2),
        "power_kw": power_kw,
        "power_factor": POWER_FACTORS.get(cat, 0.3),
    }
...
if "boxL" not in st.session_state:
    st.session_state["boxL"] = 0.0
    st.session_state["boxW"] = 0.0
    st.session_state["boxH"] = 0.0
...
if "opt_text" not in st.session_state:
    st.session_state["opt_text"] = ""
...
for idx in range(1, total + 1):
    with st.expander(f"옵션 {idx}", expanded=(idx == 1)):
          ...
          function bind() {
            try {
              labels.forEach((lab, i) => {
                const label = lab.replace("%%d", "%d");
                const inp = window.parent.document.querySelector(`input[aria-label="${label}"]`);
                if (inp && !inp.dataset.__wb_bound) {
                  inp.dataset.__wb_bound = "1";
                  inp.addEventListener('keydown', function(e) {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      const nextText = labels[i+1];
                      if (nextText) {
                        const nlabel = nextText.replace("%%d", "%d");
                        const nx = window.parent.document.querySelector(`input[aria-label="${nlabel}"]`);
                        if (nx) nx.focus();
                      }
                    }
                  });
        ...
        st.caption(
            f"치수({src}): {dims_used[0]:.1f}×{dims_used[1]:.1f}×{dims_used[2]:.1f} cm · "
            f"순 {r['net_kg']}kg / 총 {r['gross_kg']}kg · 부피(5000/6000) {r['vol_5000']}/{r['vol_6000']}kg · 신뢰 {r['confidence']}%"
        )
        ...
        rows.append({
            ...
            "category": r["category"],
            "capacity_L": extract_capacity_L(name) or global_cap,
            "power_kW": r["power_kw"],
            "box_cm": f"{dims_used[0]:.1f}x{dims_used[1]:.1f}x{dims_used[2]:.1f}",
            "dims_source": src,
            "net_kg": r["net_kg"],
            "gross_kg": r["gross_kg"],
            "vol_5000": r["vol_5000"],
